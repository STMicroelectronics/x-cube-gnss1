  ...
  if (ES_WIFI_ListAccessPoints(&EsWifiObj, &esWifiAPs) == ES_WIFI_STATUS_OK)
  {
    if (esWifiAPs.nbr > 0)
    {
      ...
      for (APCount = 0; APCount < APs->count; APCount++)
      {
        ...
        strncpy((char *)APs->ap[APCount].SSID, (char *)esWifiAPs.AP[APCount].SSID,
...
WIFI_Status_t WIFI_SendDataTo(uint8_t socket, uint8_t *pdata, uint16_t Reqlen, uint16_t *SentDatalen,
                                   ...
                                   uint32_t Timeout, uint8_t *ipaddr, uint16_t *port)
{
  ...
  if (ES_WIFI_ReceiveDataFrom(&EsWifiObj, socket, pdata,
                              Reqlen, RcvDatalen, Timeout, ipaddr, port) == ES_WIFI_STATUS_OK)
  {
    ret = WIFI_STATUS_OK;
  }
  return ret;
...
WIFI_Status_t WIFI_GetModuleFwRevision(char *rev)
{
  ...
  if (strncpy(rev, (char *)EsWifiObj.FW_Rev, ES_WIFI_FW_REV_SIZE) != NULL)
  {
    ret = WIFI_STATUS_OK;
  }
  return ret;
...
WIFI_Status_t WIFI_GetModuleID(char *Id)
{
  ...
  if (strncpy(Id, (char *)EsWifiObj.Product_ID, ES_WIFI_PRODUCT_ID_SIZE) != NULL)
  {
    ret = WIFI_STATUS_OK;
  }
  return ret;
...
WIFI_Status_t WIFI_GetModuleName(char *ModuleName)
{
  ...
  if (strncpy(ModuleName, (char *)EsWifiObj.Product_Name, ES_WIFI_PRODUCT_NAME_SIZE) != NULL)
  {
    ret = WIFI_STATUS_OK;
  }
  return ret;
